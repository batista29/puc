--COPIE e COLE AS INSTRUÇÕES A SEGUIR:

-- PASSO 1:  Execute as instruções para remover possíveis tabelas com os mesmos nomes existentes
...
--COPIE e COLE AS INSTRUÇÕES ATÉ AQUI

SELECT LOWER(NOME) AS NOMES FROM TB_CLIENTES;
SELECT UPPER(NOME) AS NOMES FROM TB_PRODUTOS;
SELECT INITCAP(NOME) AS NOMES FROM TB_CLIENTES;

UPDATE TB_CLIENTES SET NOME = LOWER(NOME);
UPDATE TB_PRODUTOS SET SABOR = UPPER(SABOR);

SELECT AVG(IDADE), CIDADE FROM TB_CLIENTES GROUP BY CIDADE;
SELECT MAX (PRECO_LISTA) AS MAIOR_PRECO, EMBALAGEM FROM TB_PRODUTOS GROUP BY EMBALAGEM;
SELECT ROUND(MIN (PRECO_LISTA),2) AS MENOR_PRECO FROM TB_PRODUTOS WHERE EMBALAGEM = 'Lata';
SELECT COUNT (PRODUTO), EMBALAGEM FROM TB_PRODUTOS GROUP BY EMBALAGEM;
SELECT COUNT (PRODUTO), SABOR FROM TB_PRODUTOS GROUP BY SABOR;

SELECT SUM(LIMITE_CREDITO) AS SOMA_LIMITES FROM TB_CLIENTES;
SELECT MAX(LIMITE_CREDITO) AS MAIOR_LIMITE FROM TB_CLIENTES;
SELECT AVG(LIMITE_CREDITO) AS MEDIA FROM TB_CLIENTES;
SELECT MIN(LIMITE_CREDITO) AS MINIMO_LIMITE FROM TB_CLIENTES;

UPDATE TB_CLIENTES SET LIMITE_CREDITO = LIMITE_CREDITO + ((LIMITE_CREDITO*5)/100);
